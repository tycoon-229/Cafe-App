...
class AdminController extends GetxController {
  ...
  void calculateStats() {
    ...
    totalActiveUsers.value =
        users.where((e) => e['is_active'] == true).length;
    totalBlockedUsers.value =
        users.where((e) => e['is_active'] == false).length;
    totalPendingAccounts.value =
        users.where((e) => e['account_status'] == 'pending').length;
    totalCafePending.value = users.where((e) {
      final cafes = e['cafes'] as List?;
      final cafe = cafes != null && cafes.isNotEmpty ? cafes.first : null;
      return cafe != null && cafe['approval_status'] == 'pending';
    }).length;
  }
  ...
  Future<void> approveAccount(String userId) async {
    try {
      await supabase
          .from('profiles')
          .update({'account_status': 'approved'}).eq('id', userId);
  ...
  Future<void> refreshUsers() async => await loadUsers();

  // =======================
  // LOGOUT
  // Dùng AuthController.logout() thay vì tự xử lý
  // để đảm bảo logic đồng nhất (xóa controller, channel, navigate)
  // =======================
